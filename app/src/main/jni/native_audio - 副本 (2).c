...
void bit_to_byte(int8_t * byte_in, short bit_out[]) {
	int bit_index;
...
void byte_to_bit(short bit_in[], int8_t byte_out[]) {
	int bit_index;

	for(bit_index = 0 ; bit_index < 130 ; bit_index = bit_index + 8) {
		//PJ_LOG(3, (THIS_FILE, "bit2byte 000 "));
		byte_out[bit_index / 8] = (bit_in[0 + bit_index] & 0x0001 || bit_in[0 + bit_index] & 0x0100) <<7 |
								   (bit_in[1 + bit_index] & 0x0001 || bit_in[1 + bit_index] & 0x0100) <<6 |
								   (bit_in[2 + bit_index] & 0x0001 || bit_in[2 + bit_index] & 0x0100) <<5 |
								   (bit_in[3 + bit_index] & 0x0001 || bit_in[3 + bit_index] & 0x0100) <<4 |
								   (bit_in[4 + bit_index] & 0x0001 || bit_in[4 + bit_index] & 0x0100) <<3 |
								   (bit_in[5 + bit_index] & 0x0001 || bit_in[5 + bit_index] & 0x0100) <<2 |
								   (bit_in[6 + bit_index] & 0x0001 || bit_in[6 + bit_index] & 0x0100) <<1 |
								   (bit_in[7 + bit_index] & 0x0001 || bit_in[7 + bit_index] & 0x0100) <<0 ;
		//PJ_LOG(5, (THIS_FILE, "byte_out[bit_index / 8 ]:%d, bit_index:%d", byte_out[bit_index / 8], bit_index));
	}
	byte_out[bit_index / 8] = (bit_in[0 + bit_index] & 0x0001 || bit_in[0 + bit_index] & 0x0100) <<7;
...
void *decode(void *argv)
{
	...
	memset(pcap_buffer, 0, sizeof(PCAP_BUFFER_SIZE));
	memset(&audio_buffers, 0, sizeof(ADUIO_BUFFERS));
	memset(&pcm_buffers, 0, sizeof(PCM_BUFFERS));
	int8_t * audio_buffer = NULL;
	pcm_buffers.size = 0;
	u_char* pcm_buffer = NULL;
								...
								&& switchUint32(ip_hdr->ip_dest) == info_str->ip_dst){//该ACELP包来自同一同步信源
									if(rtp_seq == 0){
										...
										if(acelpHandle.libAcelp != NULL) {
											...
											LOGE("success dlopen libAcelp! InitDecoderTetra:%p DecodTetra:%p", acelpHandle.InitDecoderTetra, acelpHandle.DecodTetra);
										}else{
										...
										acelpHandle.new_speech = acelpHandle.getNew_speech();
									...
									if(parse_method_type == 2){
										...
										//fwrite(pos, 1, 38, fileAudio);	
										if(audio_buffers.buffer_size == 0){
											audio_buffers.buffer_size = acelp_buffer_size * acelp_frames_per_pkt;
										}

										//LOGE("lideshou *** audio_buffers.buffer_size:%d", audio_buffers.buffer_size);

										audio_buffer = (int8_t *)malloc(1*audio_buffers.buffer_size);
										memset(audio_buffer, 0, audio_buffers.buffer_size);
										memcpy(audio_buffer, pos + acelp_frame_first_padding, audio_buffers.buffer_size);
										setAudioBuffer(&audio_buffers, audio_buffer, switchUshort(rtp_hdr->seq));
										audio_index++;
										LOGE("lideshou *** audio_buffers.size:%d audio_index:%d pkt_count:%d", audio_buffers.size, audio_index, info_str->pkt_count);

										if(audio_buffers.size >= 20 || audio_index >= info_str->pkt_count){
											//--------------------------------------------------------------------------------------------------------------
											while(audio_buffers.size > 10 || (audio_buffers.size > 0 && audio_index >= info_str->pkt_count)){
												LOGE("lideshou *** seq:%d audio_buffers.size:%d acelp_buffer_size:%d acelp_frames_per_pkt:%d acelp_samples_per_pkt:%d", audio_buffers.seq[0], audio_buffers.size, acelp_buffer_size, acelp_frames_per_pkt, acelp_samples_per_pkt);
												if(audio_buffers.seq[0] != 0){
													pollAudioBuffer(&audio_buffers, &audio_buffer);
													//fwrite(audio_buffer, 1, 36, fileAudio);
													memset(acelp_pcm_buf, 0, 480 * 2);
													//LOGE("lideshou *** %x %x %x %x %x %x %x %x",audio_buffer[0],audio_buffer[1],audio_buffer[2],audio_buffer[3],audio_buffer[4],audio_buffer[5],audio_buffer[6],audio_buffer[7],audio_buffer[8]);
													for(int i = 0; i < acelp_frames_per_pkt; i++){
														// LOGE("h26376 acelp_frames_per_pkt = %d i = %d",acelp_frames_per_pkt,i);
														memset(acelp_bit_buf, 0, ACELP_BIT_SIZE * 2);
														memset(acelp_input_buf, 0, ACELP_INPUT_SIZE * 2);
														memset(acelp_parm, 0, ACELP_PARM_SIZE * 2);
														bit_to_byte(audio_buffer + i * acelp_buffer_size, acelp_bit_buf + 1);
														for(int t = 0; t < ACELP_INPUT_SIZE; t++) {
															acelp_input_buf[t] = acelp_bit_buf[t];
														}
														...
														acelpHandle.DecodTetra(acelp_parm, acelp_pcm_buf + i * 240);
														acelpHandle.PostProcess(acelp_pcm_buf + i * 240, 240);
													}
													//fwrite(acelp_pcm_buf, 1, 960, filePCM);
													
													for(int j = 0; j < acelp_samples_per_pkt; j++){
														// LOGI("h26376 acelp_samples_per_pkt = %d j = %d",acelp_samples_per_pkt,j);
														pcm_buffers.buffer_size = sizeof(short) * SAMPLE_SIZE;														
														pcm_buffer=(u_char *)malloc(1*pcm_buffers.buffer_size);
														memset(pcm_buffer, 0, pcm_buffers.buffer_size);
														memcpy(pcm_buffer, acelp_pcm_buf + j * SAMPLE_SIZE, pcm_buffers.buffer_size);
														fwrite(pcm_buffer, 1, pcm_buffers.buffer_size, filePCM);
														pcm_index++;
														LOGI("pcm_index:%5d\t packet size:%d  %d",pcm_index,pcm_buffers.buffer_size,pcm_buffers.size);
														//################获取mutex#################
													...
													LOGE("lideshou *** pkt loss: %d", audio_buffers.seq[1]);
													pollAudioBuffer(&audio_buffers,&audio_buffer);
													for(int i = 0; i < acelp_frames_per_pkt; i++){
														pcm_buffer=(u_char *)malloc(1*pcm_buffers.buffer_size);
														memset(pcm_buffer, 0, 1*pcm_buffers.buffer_size);
